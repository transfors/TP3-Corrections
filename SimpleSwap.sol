...
contract SimpleSwap is ReentrancyGuard {
        ...
        returns (uint256 amountAWithdrawn, uint256 amountBWithdrawn)
    {
        ...
        if (liquidityToBurn == 0 || ctx.userLiquidity < liquidityToBurn) {
            emit LiquidityRemoved(msg.sender, 0, 0, block.timestamp);
            return (0, 0);
        }
        ...
        amountAWithdrawn =
            (liquidityToBurn * ctx.reserveA) /
            ctx.totalLiquidity;
        amountBWithdrawn =
            (liquidityToBurn * ctx.reserveB) /
            ctx.totalLiquidity;
